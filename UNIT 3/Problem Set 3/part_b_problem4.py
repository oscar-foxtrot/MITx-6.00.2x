class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        success = resistantToAll \
            and random.random() < self.maxBirthProb * (1 - popDensity)
            
        if success:
            newresistances = self.resistances.copy()
            
            for i in newresistances:
                if random.random() < self.mutProb: # There happens a mutation
...
class TreatedPatient(Patient):
    ...
    def __init__(self, viruses, maxPop):
        ...
        """
        Patient.__init__(self, viruses.copy(), maxPop)
        self.medication = {}
    ...
    def update(self):
        ...
        integer)
        """
        tmpviruses = self.viruses.copy()
        popDensity = len(self.viruses) / self.maxPop
        # Note that popDensity does not update when a virus is added here.
        ...
        for i in tmpviruses:
            if i.doesClear():
                ...
                try:
                    self.viruses += [i.reproduce(popDensity, self.medication)]
